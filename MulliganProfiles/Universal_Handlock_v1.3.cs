...
namespace SmartBotUI.Mulligan
{
    ...
    public class bMulliganProfile : MulliganProfile
    {
        ...
        public void findGoodDemon(List<Card.Cards> Choices, int MG, int LJ, int DG, int DI)
        {
            ...
            if (!hasGoodDemon && (Choices.Any(c => c.ToString() == _demonList.FirstOrDefault(x => x.Value == 1).Key)))
            {
                _whiteList.AddOrUpdate(_demonList.FirstOrDefault(x => x.Value == 1).Key, false);
                hasGoodDemon = true;
            }
            else if (!hasGoodDemon && (Choices.Any(c => c.ToString() == _demonList.FirstOrDefault(x => x.Value == 2).Key)))
            {
                _whiteList.AddOrUpdate(_demonList.FirstOrDefault(x => x.Value == 2).Key, false);
                hasGoodDemon = true;
            }
            else if (!hasGoodDemon && (Choices.Any(c => c.ToString() == _demonList.FirstOrDefault(x => x.Value == 3).Key)))
            {
                _whiteList.AddOrUpdate(_demonList.FirstOrDefault(x => x.Value == 3).Key, false);
                hasGoodDemon = true;
            }
            else if (!hasGoodDemon && (Choices.Any(c => c.ToString() == _demonList.FirstOrDefault(x => x.Value == 4).Key)))
            {
        ...
        public List<Card.Cards> HandleMulligan(List<Card.Cards> Choices, Card.CClass opponentClass, Card.CClass ownClass)
        {
            ...
            if (Choices.Any(c => c.ToString() == _mountainGiant) && Choices.Any(c => c.ToString() == _twilightDrake))
                strongHand = true;
            //only 1 good 4 drop 
            if (Choices.Any(c => c.ToString() == _mountainGiant) || Choices.Any(c => c.ToString() == _twilightDrake))
                goodHand = true;
            //Check if Voidcaller is in hands
            if (Choices.Any(c => c.ToString() == _voidCaller))
                voidcaller = true;
            //Check if there are no good drops in the opening hand
            if (Choices.All(c => c.ToString() != _mountainGiant) && Choices.All(c => c.ToString() != _twilightDrake) && Choices.All(c => c.ToString() != _ancientWatcher) && Choices.All(c => c.ToString() != _sunfuryProtector) && !Choices.Any(c => c.ToString() == _voidCaller))
            {
                ...
                case Card.CClass.WARLOCK:
                    {
                        ...
                        _whiteList.AddOrUpdate(_moltenGiant, true);
                        if ((Choices.Any(c => c.ToString() == _mortalCoil) && Choices.Any(c => c.ToString() == _ironbeakOwl)))
                        {
                            _whiteList.AddOrUpdate(_ironbeakOwl, false);
                            _whiteList.AddOrUpdate(_mortalCoil, false);
                        }
